...
export function RedCap({ close }: { close: () => void }) {
  ...
  useEffect(() => {
    ...
    const actions = [
      ...
      () =>
        new Promise<void>((resolve, reject) =>
          validateStudy(study) ? resolve() : reject("Study is invalid")
        ),
      // Upload
      async () => {
        const result = await createRedcapProject(username!, study);
    ...
    <div>
      ...
      {step === -1 ? (
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500">
            Please enter your RedCap username. It must exist in RedCap already.
            If you don't have one, contact Manuel.
          </p>
        </div>
      ) : (
      ...
      {step === 4 ? (
        <div className="mt-5 sm:mt-6">
          ...
          >
            {alreadyExists ? "Go to existing project" : "Visit RedCap"}
          </a>
        </div>
      ) : (
        <div className="mt-5 sm:mt-6">
